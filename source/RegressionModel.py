    ...
    def apply_func(self, inputs, name_scope=None):
        with tf.name_scope(self.name_prefix if name_scope is None else name_scope) as scope:
            ...
            if self.has_prior_pred and prior_pred is not None:
                # NaN-safe missing-data handling on the prior prediction.
                # See RNNModel.NLRegRNNCell.call for the same pattern: equality
                # against NaN is useless (NaN != NaN), and multiplicative masking
                # can't recover from upstream NaN (0 * NaN = NaN). We use
                # is_finite for NaN markers and clean the prior before any add.
                if self.missing_marker is not None:
                    mm = self.missing_marker
                    if isinstance(mm, float) and np.isnan(mm):
                        isOk_elem = tf.math.is_finite(prior_pred)
                    else:
                        mask_value_cast = tf.constant(mm, dtype=prior_pred.dtype, name='missing_marker')
                        isOk_elem = tf.not_equal(prior_pred, mask_value_cast)
                    # Clean prior so add stays finite even where hasPrior=0.
                    prior_pred_safe = tf.where(
                        isOk_elem, prior_pred, tf.zeros_like(prior_pred), name='prior_safe')
                    hasPrior = tf.cast(
                        tf.math.reduce_all(isOk_elem, axis=1),
                        dtype=prior_pred.dtype, name='prior_missing_mask')
                else:
                    prior_pred_safe = prior_pred
                    hasPrior = tf.ones_like(prior_pred, dtype=prior_pred.dtype, name='missing_mask')[:, 0]
                ...
                if self.prior_pred_op == 'add':
                    check_op = tf.Assert(~tf.math.reduce_all(prior_pred_safe == 1), ['Prior is always exactly 1! Might be a bug in passing the prior!'], name='CHECK_NONONE')
                    # Add the (cleaned, gated) prior. With cleaned prior, this is finite.
                    out += tf.cast(prior_pred_safe * hasPriorExpand, dtype=out.dtype)
                elif self.prior_pred_op == 'multiply':
                    check_op = tf.Assert(~tf.math.reduce_any(prior_pred_safe < 0), ['Prior is negative for poisson! Probably a bug in passing the prior!', prior_pred_safe], name='CHECK_NONNEGATIVE')
                    check_op2 = tf.Assert(~tf.math.reduce_all(prior_pred_safe == 0), ['Prior is always zero for poisson! Probably a bug in passing the prior!'], name='CHECK_NONZERO')
                    out *= tf.cast(prior_pred_safe, dtype=out.dtype) # Useful for outdist=poisson with exponential activation function
