...
class NLRegRNNCell(tf.keras.layers.Layer):
    ...
    def call(self, input_at_t, states_at_t):
        ...
        # ---------------------------------------------------------------------
        # NaN-safe missing-data handling.
        #
        # The pre-patch logic used `tf.not_equal(ny_in, marker)` and multiplicative
        # masking. With marker=NaN, IEEE 754 makes NaN != NaN evaluate True, so the
        # mask never fired AND `applyRecursion` saw raw NaN inputs, producing NaN
        # states that then poisoned everything via `1 * NaN = NaN`. Silent weight
        # divergence to NaN within a few epochs.
        #
        # Patched semantics:
        #   1. Detect missing entries with `tf.math.is_finite` when marker is NaN
        #      (equality is useless on NaN), `tf.not_equal` for finite sentinels.
        #   2. Replace missing entries in `ny_in` with zero BEFORE `applyRecursion`,
        #      so the recursion arithmetic stays finite regardless of marker type.
        #   3. The multiplicative `hasInputExpand * states_at_t_plus_1` gating below
        #      is then safe (state is guaranteed finite), and selects between the
        #      input-driven path and the forward-only path as before.
        # ---------------------------------------------------------------------
        if self.missing_marker is not None:
            mm = self.missing_marker
            if isinstance(mm, float) and np.isnan(mm):
                isOk_elem = tf.math.is_finite(ny_in_at_t)
            else:
                mask_value_cast = tf.constant(mm, dtype=ny_in_at_t.dtype, name='missing_marker')
                isOk_elem = tf.not_equal(ny_in_at_t, mask_value_cast)
            # Clean input: replace missing entries with 0.0. applyRecursion gets a
            # fully finite tensor so state arithmetic never produces NaN.
            ny_in_safe = tf.where(isOk_elem, ny_in_at_t, tf.zeros_like(ny_in_at_t), name='Yk_safe')
            hasInput = tf.cast(tf.math.reduce_all(isOk_elem, axis=1),
                               dtype=ny_in_at_t.dtype, name='missing_mask')  # (batch,)
        else:
            ny_in_safe = ny_in_at_t
            hasInput = tf.ones_like(ny_in_at_t, dtype=ny_in_at_t.dtype, name='missing_mask')[:, 0]
        hasInputExpand = tf.expand_dims(hasInput, axis=1, name='not_missing')
        noInputExpand = tf.identity(1-hasInputExpand, name='missing')
        # The case of non-missing data: pass the cleaned input so state stays finite.
        states_at_t_plus_1, states_at_t_plus_1_forC = self.applyRecursion(prev_states, ny_in_safe) # For the RNN case, both outputs are the same thing. For LSTM case, the first output has both main state and carry concatenated while the r2nd output contains only the main part of state that is interpreted as LSTM's output
...
class NLRegRNNCellNoA(NLRegRNNCell): # This is a dummy cell for tests. It directly connects K to C to output (ignores A and K)
    ...
    def call(self, input_at_t, states_at_t):
        ...
        prev_states = states_at_t[0]
        # NaN-safe missing handling (see NLRegRNNCell.call for full rationale).
        if self.missing_marker is not None:
            mm = self.missing_marker
            if isinstance(mm, float) and np.isnan(mm):
                isOk_elem = tf.math.is_finite(ny_in_at_t)
            else:
                mask_value_cast = tf.constant(mm, dtype=ny_in_at_t.dtype)
                isOk_elem = tf.not_equal(ny_in_at_t, mask_value_cast)
            ny_in_safe = tf.where(isOk_elem, ny_in_at_t, tf.zeros_like(ny_in_at_t))
            hasInput = tf.cast(tf.math.reduce_all(isOk_elem, axis=1), dtype=ny_in_at_t.dtype)
        else:
            ny_in_safe = ny_in_at_t
            hasInput = tf.ones(tf.shape(ny_in_at_t)[:1], dtype=ny_in_at_t.dtype)
        hasInputExpand = tf.expand_dims(hasInput, axis=1)
        # if self.unifiedAK: # Apply unified A,K
        #     states_at_t_plus_1 = self.A.apply_func(tf.keras.layers.concatenate([prev_states, ny_in_safe],axis=-1))
        #     # TO DO: add support for missing samples!
        # else: # Apply separate additive A and K
        K_y = tf.multiply(hasInputExpand, self.K.apply_func(ny_in_safe, name_scope='K')) # Element-wise multiply (input cleaned)
