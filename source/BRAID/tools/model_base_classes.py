...
class EarlyStoppingWithMinEpochs(tf.keras.callbacks.EarlyStopping):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.wait >= self.patience or np.isnan(current):
            if np.isnan(current):
                logger.info('epoch {} has blown up nan weights!'.format(epoch+1))
                self.blown_up = True
            else:
                logger.info('Early Stopping hit after {} epochs!'.format(epoch+1))
...
# updates the learning rate. This would need to be added as a sepearate callback. E.g.:
# callbacks_this.append(CustomLearningRateScheduler())
# Don't mix this with the learning rate schedulers from tf.keras which are already integrated in the code
class CustomLearningRateScheduler(tf.keras.callbacks.Callback):
    def __init__(self, patience=10, reduce_factor=0.5, min_lr=1e-4, threshold=1e-4):
        super(CustomLearningRateScheduler, self).__init__()
        self.patience_sc = patience  # Number of epochs to wait before reducing learning rate
        self.reduce_factor = reduce_factor  # Factor by which to reduce the learning rate
        self.min_lr = min_lr  # Minimum learning rate
        self.threshold_sc = threshold  # Threshold for significant improvement
        self.wait_sc = 0  # Counter to keep track of epochs without significant improvement
        self.best_sc = float('inf')  # Best training loss
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        improvement = (self.best_sc - current_loss) / self.best_sc
        if improvement > self.threshold_sc or np.isinf(self.best_sc):
            self.best_sc = current_loss
            self.wait_sc = 0
        else:
            self.wait_sc += 1
            if self.wait_sc >= self.patience_sc:
                ...
                self.wait_sc = 0  # Reset wait counter
'''
...
class ModelWithFitWithRetry:
    ...
    def fit_with_retry(self, init_attempts=1, 
        ...
        if callbacks is None:
            callbacks = []
        # Early stopping:
        # callbacks.append(EarlyStoppingWithMinEpochs(
        #     monitor=early_stopping_measure, patience=early_stopping_patience, restore_best_weights=False, start_from_epoch=start_from_epoch
        # )) 
        ...
        def compute_reg_loss(epoch, logs=None):
            if logs is None:
                return
            if self.lr_scheduler_name is not None and self.lr_scheduler_args is not None:
                update_iter = self.model.optimizer.iterations.numpy()
                logs['learning_rate'] = self.model.optimizer.learning_rate(update_iter).numpy()
            else:
                logs['learning_rate'] = float(self.model.optimizer.learning_rate) # Save current learning rate: would not work with learning rate scheduler
            if len(self.model.losses): # If we have some regularization loss
                ...
                if 'val_loss' in logs:
                    logs['val_loss_minus_regularization'] = logs['val_loss'] - regularization_loss
        # callbacks.append(tf.keras.callbacks.LambdaCallback(
        #     on_epoch_end=compute_reg_loss
        # ))

        latest_nonnan_weights, latest_nonnan_weights_epoch, latest_epoch = None, None, None
        def keep_latest_nonnan_weights_callback(epoch, logs):
            ...
            if len(nan_weights) == 0:
                ...
                logger.info(f'epoch {epoch} has {len(nan_weights)} blown up nan weights!')
                self.blown_up = True
            latest_epoch = epoch
        ...
        while attempt < init_attempts:
            ...
            if self.log_dir != '':
                ...
                if tb_make_prediction_plots or tb_make_prediction_scatters:
                    ...
                    if isinstance(self, RegressionModel): # For RegressionModel
                        ...
                        y_in = y[0]
                        yAll = np.reshape(y_in, (int(y_in.size/y_in.shape[-1]), y_in.shape[-1]), 'C')
                    if validation_data is not None:
                        x_val, y_val = validation_data[0], validation_data[1]
                        if isinstance(self, RegressionModel): # For RegressionModel
                            yAll_val = y_val
                        else: # For RNNModel
                            yAll_val = np.reshape(y_val[0], (int(y_val[0].size/y_in.shape[-1]), y_in.shape[-1]), 'C')
                    ny = y_in.shape[-1]
                    nyIndsToPlot = np.array( np.unique( np.round(np.linspace(0,ny-1,2)) ), 'int')
                    fig1, fig2, fig3, fig4 = None, None, None, None
                    def tensorboard_plot_signals(epoch, logs):
                        nonlocal fig1, fig2, fig3, fig4
                        if epoch!=0 and epoch%tb_plot_epoch_mod < tb_plot_epoch_mod-1:
                            return
                        steps_ahead = [1] if not hasattr(self, 'steps_ahead') or self.steps_ahead is None else self.steps_ahead
                        ...
                        if isinstance(self, RegressionModel): # For RegressionModel
                            ...
                            yHat = [np.reshape(yHatThis, yAll.shape) for yHatThis in list(yHat)]
                            yHat = list(shift_ms_to_1s_series(yHat, steps_ahead, missing_marker=np.nan, time_first=True))
                            predLegStrs = [f'Pred {step_ahead}-step' for step_ahead in steps_ahead]
                            batch_count = int(x[0].shape[0]/self.batch_size)
                            batchCntStr = ', {} batches'.format(batch_count)
                        titleHead = 'Epoch:{}{} (training data)\n'.format(epoch, batchCntStr)
                        if tb_make_prediction_plots:
                            if fig1 is not None:
                                fig1.clf()
                            plotArgs = {'missing_marker': self.missing_marker, 'addNaNInTimeGaps': False, 
                                'plotDims': nyIndsToPlot, 
                                'predLegStrs': predLegStrs, 'y_pred_is_list': True, 
                                'lineStyles': ['-', '-', '--', '-.', ':'],
                        ...
                        if tb_make_prediction_scatters:
                            ...
                            scatterArgs = {
                                ...
                                'figsize': (11,4), 'title': ['Dim{} '.format(di) for di in nyIndsToPlot],
                                'legNames': [f'{step_ahead}-step' for step_ahead in steps_ahead],
                                'addPerfMeasuresToLegend': ['CC', 'R2'],
                                'addPerfMeasuresToTitle': ['CC', 'R2', 'MSE'], 'return_fig': True
                            }
                        ...
                        if validation_data is not None:
                            if isinstance(self, RegressionModel): # For RegressionModel
                                ...
                                yHat_val = self.predict_with_keras(x_val)[:len(steps_ahead)]
                                yHat_val = [np.reshape(yHatThis, yAll_val.shape) for yHatThis in list(yHat_val)]
                                yHat_val = list(shift_ms_to_1s_series(yHat_val, steps_ahead, missing_marker=np.nan, time_first=True))
                                batchCntStr = ', {} batches'.format( int(x_val[0].shape[0]/self.batch_size) )
            ...
            if eagerly_flag_backup:
                logger.warning('Tensorflow was set up globally to run eagerly. This is EXTREMELY slow so we have temporarily disabled it and will reenable it after model fitting. Consider fixing this global setting by running tf.config.run_functions_eagerly(False).')
            if self.model.run_eagerly or tf.config.functions_run_eagerly():
                warnings.warn('This Tensorflow model is set up to run eagerly. This will be EXTREMELY slow!!! Please fix.')
            ...
            if 'verbose' in kwargs and kwargs['verbose'] != 2:
                ...
                if 'regularization_loss' in history.history: 
                    ...
                    reg_to_total_change_ratio = np.diff(reg_loss_range)[0] / np.diff(loss_range)[0]
                    median_reg_to_total_ratio = np.median(reg_loss / total_loss)
                    logger.info('{:.2g}% of the changes in total loss ({:.2g} => {:.2g}) are due to changes in regularization loss ({:.2g} => {:.2g})'.format(reg_to_total_change_ratio*100, loss_range[1], loss_range[0], reg_loss_range[1], reg_loss_range[0]))
                    logger.info('Median ratio of reg_loss to total_loss is {:.2g}%'.format(median_reg_to_total_ratio*100))
                    ...
                    if reg_to_loss_ratio > 0.5:
                        logger.info('Regularization lambda is too high, regularization is dominating the total loss')
                    elif reg_to_loss_ratio < 0.01:
                        logger.info('Regularization lambda is too low, regularization is an almost negligible part (<1%) of the total loss')
            logger.info('Model fitting took {:.2f}s'.format(fitTime))
            weights = self.model.get_weights()
            nan_weights = [v for v in weights if np.any(np.isnan(v))]
            if len(nan_weights) > 0 and keep_latest_nonnan_weights and latest_nonnan_weights is not None:
                logger.warning(f'{len(nan_weights)} weights had nans, replacing with weights from the latest epoch with non-nan weights (epoch {latest_nonnan_weights_epoch})')
                self.model.set_weights(latest_nonnan_weights)
                epoch_ind = [ep for ep in history.epoch if ep <= latest_nonnan_weights_epoch][-1]
            ...
            if init_attempts > 1:
                ...
                if attempt == init_attempts: # Select final model
                    lossAll = [np.array(h.history[early_stopping_measure])[
                               np.where(np.array(h.epoch)==h.params['picked_epoch'])[0]
                               ][0] for h in historyAll]
                    ...
                    logger.info('Selected model from learning attempt {}/{}, which had the smallest loss ({:.8g})'.format(1+bestInd, init_attempts, lossAll[bestInd]))
