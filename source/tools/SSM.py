...
class SSM(LSSM):
    ...
    # def generateObservationFromStates(self, X, u=None, param_names=['C', 'D'], prep_model_param='', mapping_param=''):
        ...
        if C is not None or D is not None:
            ny = self.get_param_io_count(param_names[0])[1] if C is not None else self.get_param_io_count(param_names[1])[1]
            N = X.shape[0]
            Y = np.zeros((N, ny))
            if C is not None:
            ...
            if D is not None and u is not None:
                ...
                # DUThis =  self.apply_param(param_names[1], u[step_ahead-1:,:].T).T
    ...
    def generateRealizationWithQRS(self, N, x0=None, w0=None, u0=None, u=None, wv=None, return_z=False, return_z_err=False, return_wv=False, \
            blowup_threshold=None, reset_x_on_blowup=None, randomize_x_on_blowup=None):
    ...
    def generateRealizationWithKF(self, N, x0=None, u0=None, u=None, e=None, return_z=False, return_z_err=False, return_e=False, \
        ...
        for i in tqdm(range(N), 'Generating realization'):
            ...
            print_secs = 60
            if (toc-tic) > print_secs and np.mod(toc-tic, print_secs) < 0.5*print_secs and np.mod(time_passed, print_secs) >= 0.5*print_secs:
                logger.info('{:.2f}% ({}/{} samples) generated after {:.3g} min(s) and {:.3g} second(s)'.format(i/N*100, i, N, (toc-tic)//60, (toc-tic)%60))
            time_passed = toc-tic
    ...
    def find_fixedpoints(self, Y=None, X=None, U=None, N=None):
        ...
        oDiff = (X[inds[1:],:].T - self.apply_param('K', Y[inds[:-1],:].T) - X[inds[:-1], :].T).T
        ...
        if rootVal.size > 0: 
            ...
            if rootVal.size > 1000: 
                rootVal = np.random.choice(rootVal.flatten(), 1000, replace=False)[:,np.newaxis]
    ...
    def kalman(self, Y, U=None, x0=None, P0=None, steady_state=True, \
            blowup_threshold=None, clip_on_blowup=None, reset_x_on_blowup=None, randomize_x_on_blowup=None):
        if blowup_threshold is None:
            if hasattr(self, 'blowup_threshold'):
                blowup_threshold = self.blowup_threshold
            else:
        ...
        for i in tqdm(range(N), 'Estimating latent states'):
            ...
            # Check if Xp has blown up
            if np.any(np.isnan(newXp)) or np.any(np.isinf(newXp)) or np.any(np.abs(newXp) > blowup_threshold):
                ...
                if randomize_x_on_blowup:
                    newXp = np.atleast_2d(np.random.multivariate_normal(mean=np.zeros(self.state_dim), cov=self.XCov)).T
    ...
    # def propagateStates(self, allXp, step_ahead=1):
        for step in range(step_ahead-1): 
            ...
            if U is not None and hasattr(self, 'B') and self.B.size>0 and hasattr(self, 'observable_U_in_Kfw') and self.observable_U_in_Kfw:
                if hasattr(self, 'UPrepModel') and self.UPrepModel is not None:
                    U = self.UPrepModel.apply(U, time_first=True) # Apply any mean removal/zscoring
                BUThis = self.apply_param('B', U[step:,:].T).T 
