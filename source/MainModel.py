...
class MainModel(PredictorModel):
    ...
    def fit(self, Y, Z=None, U=None, nx=None, n1=None, 
            ...
            has_UFT_z=None,  ## ADDED
            has_UFT_reg_z=None,  ## ADDED
        ...
        if create_val_from_training:
            ...
            val_for_batch = Y_validation if Y_validation is not None else Z_validation
            Ndat_for_batch_size = np.min((Ndat, val_for_batch.shape[1])) if val_for_batch is not None else Ndat # Make sure batch size is not too large for validation data
        else:
        ...
        self.has_UFT_z = ((has_UFT_z if has_UFT_z is not None else has_UFT) and self.nu > 0)  ## ADDED
        self.has_UFT_reg_z = ((has_UFT_reg_z if has_UFT_reg_z is not None else has_UFT_reg) and self.nu > 0)  ## ADDED
        ...
        need_Cfw = np.any(np.array(steps_ahead)!=1) and ((self.nu > 0 and (has_UFT or has_UFT_z) and not self.observable_U_in_Cfw) or self.has_Dyz)
        self.need_Cfw = need_Cfw
        need_fw_reg_models = np.any(np.array(steps_ahead)!=1) and ((self.nu > 0 and (has_UFT_reg or has_UFT_reg_z) and not self.observable_U_in_Cfw) or self.has_Dyz)
    ...
    def prepare_inputs_to_model2_Cz(self, Y, Z, U, allX_steps, allXp2_steps, allZp_steps, steps_ahead):
        ...
        if self.nu > 0 and self.has_UFT_reg_z: 
            UReverseShifted_steps = shift_1s_to_ms_series(U, steps_ahead, self.missing_marker, time_first=False)
    ...
    def prepare_inputs_to_model1(self, Y, U): # Supported preparing n1_in. If observable_U_in_Kfw: n1_in = U_steps.
        ...
        if self.nu > 0 and self.ny > 0:
            ...
            YU = U
        if self.has_UFT_z and self.has_Dyz:
            FT_in = YU
        elif self.has_UFT_z:
            FT_in = U
        elif self.has_Dyz:
    ...
    def build_models(self):
        ...
        need_Cfw = np.any(np.array(steps_ahead)!=1) and ((self.nu > 0 and (self.has_UFT or self.has_UFT_z) and not self.observable_U_in_Cfw) or self.has_Dyz) # replace with "(self.nu > 0 and self.has_UFT and not self.observable_U_in_Cfw)" for supporting observable U in Cfw
        need_fw_reg_models = np.any(np.array(steps_ahead)!=1) and ((self.nu > 0 and (self.has_UFT_reg or self.has_UFT_reg_z) and not self.observable_U_in_Cfw) or self.has_Dyz) # replace with "(self.nu > 0 and self.has_UFT_reg and not self.observable_U_in_Cfw)" for supporting observable U in reg model
        ...
        if self.model1_Cy_Full:
            ...
            input_dim_Cy = self.n1+self.n2+self.nu if self.has_UFT_reg else self.n1+self.n2
    ...
    def restoreModels(self):
        ...
        if not hasattr(self, 'has_UFT_z'): self.has_UFT_z = getattr(self, 'has_UFT', True)
        if not hasattr(self, 'has_UFT_reg_z'): self.has_UFT_reg_z = getattr(self, 'has_UFT_reg', True)
        ...
        if self.n2 > 0:
            ...
            if not self.model1_Cy_Full:
                if self.model1_Cy is None:
                    ...
                    Cy1 = CyDy[:, :self.n1]
                    Cy2 = CyDy2[:, :self.n2]
                    Dy1 = CyDy[:, self.n1:] if CyDy.shape[1] > self.n1 else np.zeros((self.ny, self.nu))
                    Dy2 = CyDy2[:, self.n2:] if CyDy2.shape[1] > self.n2 else np.zeros((self.ny, self.nu))
                    Dy = Dy1 + Dy2  # add up any feedthrough from two stages
            ...
            if (self.model2_Cz is None and 
                ...
                Cz1 = CzDz[:, :self.n1]
                Cz2 = CzDz2[:, :self.n2]
                Dz1 = CzDz[:, self.n1:] if CzDz.shape[1] > self.n1 else np.zeros((self.nz, self.nu))
                Dz2 = CzDz2[:, self.n2:] if CzDz2.shape[1] > self.n2 else np.zeros((self.nz, self.nu))
                Dz = Dz1 + Dz2 # Add up any feedthrough from two stages
    ...
    def setToLSSM(self, sys, model1_Cy_Full=True, model2_Cz_Full=True, allow_nonzero_Cz2=True, A_KC_is_block_diagonal=False, props={},
        ...
        self.has_UFT = (has_UFT and self.nu>0)
        self.has_UFT_reg = (has_UFT_reg and self.nu>0)
        self.has_UFT_z = ((has_UFT_z if has_UFT_z is not None else has_UFT) and self.nu > 0)
        self.has_UFT_reg_z = ((has_UFT_reg_z if has_UFT_reg_z is not None else has_UFT_reg) and self.nu > 0)
        ...
        self.steps_ahead = steps_ahead
        need_Cfw = np.any(np.array(steps_ahead)!=1) and ((self.nu > 0 and (self.has_UFT or self.has_UFT_z) and not self.observable_U_in_Cfw) or self.has_Dyz)
        need_fw_reg_models = np.any(np.array(steps_ahead)!=1) and ((self.nu > 0 and (self.has_UFT_reg or self.has_UFT_reg_z) and not self.observable_U_in_Cfw) or self.has_Dyz)
        ...
        if self.n1 > 0:
            ...
            if self.has_Dyz and self.has_UFT_z:   # Dyz will be incorporated in RNN1
                ...
                nft = 0
            if self.has_UFT_z or self.has_Dyz:
    ...
    def predict(self, Y, U=None, x0=None):
        ...
        if not hasattr(self, 'has_UFT_z'):
            self.has_UFT_z = self.has_UFT
        if not hasattr(self, 'has_UFT_reg_z'):
            self.has_UFT_reg_z = self.has_UFT_reg
        ...
        self.has_UFT_reg_z = U is not None and self.nu>0 and self.has_UFT_reg_z
        ...
        if U is not None and Y is not None:
            ...
            YU = U
        if self.n1 > 0:
            if self.has_Dyz and self.has_UFT_z:
                FT_in = YU.T
            elif self.has_UFT_z: 
                FT_in = UT
            elif self.has_Dyz:
        ...
        if self.nz > 0 and self.model2_Cz is not None:
            if self.has_Dyz and (self.n1 == 0 or self.model2_Cz_Full) and self.has_UFT_reg_z:
                UOrYU = YU
            elif self.has_Dyz and (self.n1 == 0 or self.model2_Cz_Full):
                UOrYU = Y
            elif self.has_UFT_reg_z:
                UOrYU = U
            else:
                UOrYU = None
    ...
    def propagate_prediction_forward_with_data_gen(self, allXp_steps, allXp_internal_steps, allYp_steps, allZp_steps, steps_ahead_model1, steps_ahead, noise_scale=0, allU_steps=None):
        ...
        for step_ahead in steps_ahead_model1:
            if step_ahead == 1: # get 1-step ahead preds that are already computed in MainModel.predict earlier
                ...
                if allU_steps is not None and self.nu > 0:
                    latest_YpU = np.concatenate( (latest_Yp, allU_steps[step_ahead-2].T), axis=1 )
                else:
                    latest_YpU = latest_Yp
                    
                # Construct FT for Model 1 (Z prediction)
                if self.has_UFT_z and allU_steps is not None and self.nu > 0:
                    FT_this_z = allU_steps[step_ahead-1].T
                else:
                    FT_this_z = None
                    
                # Construct FT for Model 2 (Y prediction)
                if self.has_UFT and allU_steps is not None and self.nu > 0:
                    FT_this_y = allU_steps[step_ahead-1].T
                else:
                    FT_this_y = None
                ...
                if self.n2 > 0:
                    ...
                    if self.model2_Cz is not None and not self.model2_Cz_Full:
                        if allU_steps is not None and self.has_UFT_reg_z and self.nu > 0:
    ...
    def generateRealization(self, N, random_x0=False, x0=None, u=None, eY=None, eZ=None):
        ...
        for i in range(N):
            ...
            if self.nu > 0:
                ...
                # Y inputs (tied to original has_UFT flags)
                xk_uk_y = np.concatenate( (Xp1, Xp2, uk), axis=0 ) if self.has_UFT_reg else xk
                x1k_uk_y = np.concatenate( (Xp1, uk), axis=0 ) if self.has_UFT_reg else Xp1
                x2k_uk_y = np.concatenate( (Xp2, uk), axis=0 ) if self.has_UFT else Xp2

                # Z inputs (tied to new has_UFT_z flags)
                xk_uk_z = np.concatenate( (Xp1, Xp2, uk), axis=0 ) if self.has_UFT_reg_z else xk
                x1k_uk_z = np.concatenate( (Xp1, uk), axis=0 ) if self.has_UFT_z else Xp1
                x2k_uk_z = np.concatenate( (Xp2, uk), axis=0 ) if self.has_UFT_reg_z else Xp2
            else:
                xk_uk_y = xk_uk_z = xk
                x1k_uk_y = x1k_uk_z = Xp1
                x2k_uk_y = x2k_uk_z = Xp2

            if self.model1_Cy_Full:
                yk_res += self.model1_Cy.predict(xk_uk_y)
            else:
                if self.n1 > 0:
                    yk_res += self.model1_Cy.predict(x1k_uk_y)
                if self.n2 > 0:
                    yk_res += self.model2.rnn.cell.C.predict(x2k_uk_y)
            if self.model2_Cz_Full:
                zk_res += self.model2_Cz.predict(xk_uk_z)
            else:
                if self.n1 > 0:
                    zk_res += self.model1.rnn.cell.C.predict(x1k_uk_z)
                if self.n2 > 0:
                    zk_res += self.model2_Cz.predict(x2k_uk_z)
            ...
            # Construct FT_in for Model 1 (Z prediction)
            if self.has_UFT_z and self.has_Dyz:
                ftk_1 = np.concatenate((yy, uk), axis=0) if self.nu > 0 else yy
            elif self.has_UFT_z:
                ftk_1 = uk if self.nu > 0 else None
            elif self.has_Dyz:
                ftk_1 = yy
            else:
                ftk_1 = None
                
            # Construct FT_in for Model 2 (Y prediction)
            ftk_2 = uk if (self.nu > 0 and self.has_UFT) else None

            if self.n1 > 0:
                Xp1_cur, zk_next1, Xp1_next = self.model1.predict(yk_uk, initial_state=Xp1.T, FT_in=ftk_1)
            else:
                Xp1_next = np.zeros(self.n1)
            Xp1_next = Xp1_next[:, np.newaxis]

            if self.n2 > 0:
                Xp2_cur, yk_next1, Xp2_next = self.model2.predict(x1_yk_uk, initial_state=Xp2.T, n1_in=Xp1_next, FT_in=ftk_2)
