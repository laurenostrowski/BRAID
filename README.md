...
for-loop / if-statement indentation: hidden_layers, hidden_units outside the loop, so only the last regex match's var_names survives the loop – the if-block sets at most one component's _args to NL and the rest are set to {} (meaning linear)  
